...
MiRemoteirLearn = function(platform, config) {
    ...
    this.platform.log.debug("[MiIRRemote][MiRemoteirLearn][DEBUG]Initializing learn: " + this.config["ip"] + ", accessories size: " + accessoriesArr.length);
...
MiRemoteirLearnButton.prototype.getServices = function() {
    ...
    MiRemoteirLearnButtonOnCharacteristic
        .on('set',function(value, callback) {
            this.platform.log.info("[MiIRRemote][MiRemoteirLearn] Learn Started");
            if(value == true){
...
MiRemoteirLearnButton.prototype.runTimer = function() {
    ...
    if(this.upt <= 0){
        ...
        that.platform.log.info("[MiIRRemote][MiRemoteirLearn] Learn Stopped");
    }else{
        this.timekey = "123456789012345";
        if(this.upt == 4){
            this.device.call("miIO.ir_learn", {"key":this.timekey}).then(result => {
                that.platform.log.info("[MiIRRemote][MiRemoteirLearn]MiRemoteirLearn Waiting...");
            }).catch(function(err) {
                if(err == "Error: Call to device timed out"){
                    that.platform.log.debug("[MiIRRemote][ERROR]MiRemoteirLearn - Remote Offline");
                }else{
                    that.platform.log.debug("[MiIRRemote][MiRemoteirLearn][ERROR] Error: " + err);
                }
            });
        }else{
            this.device.call("miIO.ir_read", {"key":this.timekey}).then(result => {
                if(result['code'] !== ""){
                    that.platform.log.info("[MiIRRemote][MiRemoteirLearn]Learned Code: " + result['code']);
                    this.updatetimere = false;
                    this.upt = 0;
                    this.MiRemoteirLearnService.getCharacteristic(Characteristic.On).updateValue(false);
                    that.platform.log.info("[MiIRRemote][MiRemoteirLearn] Learn Success!");
                }else{
                    that.platform.log.debug("[MiIRRemote][MiRemoteirLearn][DEBUG]Learn Waiting...");
                }
            }).catch(function(err) {
                if(err == "Error: Call to device timed out"){
                    that.platform.log.debug("[MiIRRemote][ERROR]MiRemoteirLearn - Remote Offline");
                }else{
                    that.platform.log.error("[MiIRRemote][MiRemoteirLearn][ERROR] Error: " + err);
                }
                callback(err);
            });
        }
        that.platform.log.debug("[MiIRRemote][MiRemoteirLearn] " + this.upt + " Seconds left");
    }
}
